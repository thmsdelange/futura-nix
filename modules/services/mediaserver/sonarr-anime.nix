{
  flake.modules.nixos.mediaserver =
    ...
    let
      saPort = (config.hostSpec.networking.ports.${hostConfig.name}.tcp.sonarr-anime or 8990);
    ...
    {
      sops.secrets = {
        "services/sonarr-anime/password" = {
      ...
      nixflix.sonarr-anime = {
        ...
        config = {
          ...
          hostConfig.password._secret = config.sops.secrets."services/sonarr-anime/password".path;
          delayProfiles = [
            {
              enableUsenet = true;
              enableTorrent = true;
      ...
      services.caddy.virtualHosts."${subdomain}.${domain}" = {
        extraConfig = ''
          reverse_proxy 127.0.0.1:${builtins.toString saPort}
        '';
      };
