{
  flake.modules.nixos.mediaserver =
    ...
    let
      rPort = (config.hostSpec.networking.ports.${hostConfig.name}.tcp.radarr or 7878);
    ...
    in
    {
      ...
      nixflix.radarr = {
        ...
        config = {
          ...
          hostConfig.password._secret = config.sops.secrets."services/radarr/password".path;
          delayProfiles = [
            {
              enableUsenet = true;
              enableTorrent = true;
      ...
      services.caddy.virtualHosts."${subdomain}.${domain}" = {
        extraConfig = ''
          reverse_proxy 127.0.0.1:${builtins.toString rPort}
        '';
      };
